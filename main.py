...
class boss(boss_helper):
    ...
    def up_dat_mem(self):
        ...
        q_on = input("What you would like change? \n 1. Email \n 2. First Name \n 3. Second Name \n 4. Payment \n Answer: ")
        if int(q_on) == 1:
            ...
            if f_d:
                ...
                data = cursor.execute("UPDATE WR SET Email = ? WHERE First_Name= ? AND Last_Name = ? AND Email = ?",
        ...
        if int(q_on) == 2:
            ...
            if f_d:
                ...
                data = cursor.execute("UPDATE WR SET First_Name = ? WHERE First_Name= ? AND Last_Name = ? AND Email = ?",
                                      (em, f_n, s_n, e_a)).fetchall()
    ...
    #Check all information about all members of staff
    def ch_al_on(self):
        row = cursor.execute("SELECT * FROM WR")
        data = cursor.execute("SELECT * FROM WR")
        for row in data:
            print(row)
    ...
    def add_mem(self):
        f_n = input("People`s First name: ")
        s_n = input("People`s Second name: ")
        e_m = input("Create Email for him: ")
        p_s = input("Create password for him: ")
        p_t = input("His Payment: ")
        s_t = input("His status Employee/Boss_Helper/Boss: ")

        cursor.execute("INSERT INTO WR (Email, First_Name, Last_Name, Password, Payment, Status) VALUES (?, ?, ?, ?, ?, ?)",
                               (e_m, f_n, s_n, p_s, p_t, s_t))
        connect.commit()
